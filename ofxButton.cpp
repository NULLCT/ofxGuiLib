...
void ofxButton::set(int _x,int _y,int _w,int _h,ofColor& _backcolor,ofColor& _wordcolor,ofTrueTypeFont& _font,std::string _text){ //Set Button size pos color font and text
  x = _x;
  y = _y;
  w = _w;
  h = _h;
...
bool ofxButton::isfocus(void) { // if this focus => True
  return (x <= ofGetMouseX() and ofGetMouseX() <= x + w) and (y <= ofGetMouseY() and ofGetMouseY() <= y + h);
}
...
bool ofxButton::run(void) { // draw button and return bool if this click => True
  if (isfocus()){
    ...
    font.drawString(text,(w-font.stringWidth(text))/2+x,(h+font.stringHeight(text))/2+y);
